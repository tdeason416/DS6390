...
class Magma{
 ...
 Magma(){
  ...
  Magma(PApplet p, float volume, int numVolcanos){
   this.p = p;
//   this.land = new Land(this, (854/8), (480/8) );
 ...
 void flow(){
   this.state += p.random(0f, .25f);
   float flowAmt = p.abs(p.sin(p.PI * this.state)  * this.peakFlow);
   int choice =PApplet.parseInt(p.random(0, this.numVolcanos));
//   println(choice);
   this.volcanos.get(choice).addMagma(flowAmt);
   this.probs[PApplet.parseInt(p.random(0,this.probs.length))] = choice;
